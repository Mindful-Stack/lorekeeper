...
It then:
- Checks git age in each knowledge repo and flags any older than `KNOWLEDGE_MAX_AGE_DAYS` (default 7). Stale KBs are refreshed without asking the user first (the Bash permission prompt still applies — the hook can't waive it); `KNOWLEDGE_AUTO_REFRESH=0` reverts to asking first
